...
# http basic auth
[[ $HTTPBASICAUTHUSER != "" ]] && mkdir -p /etc/htpasswd && htpasswd -b /etc/htpasswd/.htpasswd $HTTPBASICAUTHPW $HTTPBASICAUTHUSER && mv /httpbasicauthvhost.conf /etc/apache2/sites-available/000-default.conf && service apache2 reload
